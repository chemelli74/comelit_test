...
T = TypeVar(
    "T",
    bound=Mapping[str, dict[int, ComelitSerialBridgeObject] | AlarmDataObject],
)
...
class ComelitBaseCoordinator(DataUpdateCoordinator[T]):
    ...
    async def _async_remove_stale_devices(
        self,
        previous_list: dict[int, ComelitSerialBridgeObject] | AlarmDataObject,
        current_list: dict[int, ComelitSerialBridgeObject] | AlarmDataObject,
        dev_type: str,
    ) -> None:
...
class ComelitSerialBridge(
    ComelitBaseCoordinator[
        dict[str, dict[int, ComelitSerialBridgeObject] | AlarmDataObject]
    ]
):
    ...
    async def _async_update_system_data(
        ...
        if self.data:
            for dev_type in (CLIMATE, COVER, LIGHT, IRRIGATION, OTHER, SCENARIO):
                assert isinstance(self.data[dev_type], dict)
        ...
        alarm_data: dict[str, AlarmDataObject] = {}
        if self.vedo_pin:
            alarm_data = await self.api.get_all_areas_and_zones()
            self.data.update(alarm_data)

        return cast(T, data | alarm_data)


class ComelitVedoSystem(ComelitBaseCoordinator[Mapping[str, AlarmDataObject]]):
